...
def _get_dist_device():
    """Return the best available device for distributed operations.
    CUDA is preferred for NCCL backend; falls back to CPU for GLOO."""
...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        if not is_dist_avail_and_initialized():
            return
        t = torch.tensor([self.count, self.total], dtype=torch.float64, device=_get_dist_device())
        dist.barrier()
        dist.all_reduce(t)
        t = t.tolist()
        self.count = int(t[0])
...
def all_gather(data):
    ...
    tensor = torch.ByteTensor(storage).to(_get_dist_device())

    # obtain Tensor size of each rank
    local_size = torch.tensor([tensor.numel()], device=_get_dist_device())
    size_list = [torch.tensor([0], device=_get_dist_device()) for _ in range(world_size)]
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(torch.empty((max_size,), dtype=torch.uint8, device=_get_dist_device()))
    if local_size != max_size:
        padding = torch.empty(size=(max_size - local_size,), dtype=torch.uint8, device=_get_dist_device())
