...
def run_predict_from_standard_rt(args):
    """
    从样品目录的 xic_matrix 按各通道 XIC（可选平滑后）最高峰 RT 裁 ROI，运行 QuanFormer 输出 prediction.csv。
    """
    ...
    if not ok:
        raise RuntimeError("QuanFormer prediction failed while generating prediction.csv")
...
def build_parser():
    ...
    p4 = sp.add_parser(
        "predict_from_ref_rt",
        help="从样品 xic_matrix 按各通道最高峰 RT 裁 ROI 并运行 QuanFormer，输出 prediction.csv（与历史子命令名兼容）。",
    )
    ...
    p4.add_argument("--model", required=True, help="QuanFormer checkpoint 路径")
