# -*- coding: utf-8 -*-
"""
QuanFormer 统一推理入口：合并 testXIC + newtest，支持单张图模式。
...
def process_single_image(
    ...
    if output_dir is None:
        tmp = tempfile.mkdtemp(prefix="quanformer_single_")
        output_dir = tmp
        if not keep_temp:
            import atexit
            import shutil
...
def main_cli():
    parser = argparse.ArgumentParser(description="QuanFormer 统一推理入口（合并 testXIC + newtest）")
    ...
    if args.mode in {"pipeline_mzml", "pipeline_batch_mzml"}:
        ...
        # 3) Run QuanFormer (newtest) in batch mode
