...
from quanformer.util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou, distance_box_iou, complete_box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_iou: float = 1, iou_type: str = 'giou'):
        """Creates the matcher
    ...
    def forward(self, outputs, targets):
        ...
        Params:
            22-10: This is a dict that contains at least these entries:
        ...
        # Compute the giou cost betwen

        if self.iou_type == "giou":
            cost_iou = -generalized_box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))
        elif self.iou_type == "diou":
            cost_iou = -distance_box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))
        elif self.iou_type == "ciou":
            cost_iou = -complete_box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))
        else:
