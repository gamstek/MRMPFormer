...
| 4 | 手搓 `argparse.Namespace` 跨模块传参 | 🟡 | **仍存在**，且**引入新问题** | pipeline 分支 [cli.py:802-816](file:///d:/work/MRMPFormer/model/inference/cli.py#L802-L816)、batch_dir 分支 [cli.py:1012-1015](file:///d:/work/MRMPFormer/model/inference/cli.py#L1012-L1015)。详见 §2.1 |
| 5 | QC 参数在阶段①/③重复下发 | 🟡 | **仍存在** | 阶段① [cli.py:770-786](file:///d:/work/MRMPFormer/model/inference/cli.py#L770-L786) 传 `extract_xic_with_pyopenms`；阶段③ [cli.py:860-861](file:///d:/work/MRMPFormer/model/inference/cli.py#L860-L861) 再传 `snr_pipeline_run`。详见 §2.2 |
| 6 | `single` 模式 ROI 独立实现副本 | 🟢 | **已消除**（随模式删除） | `_generate_single_roi`（硬编码 window_half_min=1.0、无 QC 的平行实现）已删 |

**结论：6 项中 4 项已随模式重构修复/消除，2 项仍存留（#4、#5），另发现 1 个重构新引入问题（§2.1-a）。**
...
**验证方法**：`--mode batch_dir --integration_method raw` 后检查输出文件名为 `prediction_raw.csv`（现会仍是 `prediction.csv`）。
...
`--standard_refs_csv` 仍存在三处：参数定义 [cli.py:546](file:///d:/work/MRMPFormer/model/inference/cli.py#L546)、pipeline 分支提示 [cli.py:746-749](file:///d:/work/MRMPFormer/model/inference/cli.py#L746-L749)、configs/inference_pipeline.json 注释键。按原计划在任务 3 删除，此处仅记录不动。

---

## 3. 修复顺序建议

1. **§2.1-a**（batch_dir 硬编码 integration_method）——2 行，行为修复，最优先；
2. **§2.2 方案 A**（QC 冗余下发）——2 行 + 注释；
3. **§2.1-b/c**（Namespace 工厂化）——中修，可与任务 4/5 终端输出优化合并提交；
